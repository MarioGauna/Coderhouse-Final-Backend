...
export class CartService extends baseDao{
    ...
    async addToCart(id,obj){
        try {
            ...
            if(cartExists !== null){
                let existsProd = await cartModel.findOne({[this.ID_FIELD]:id,"products._id":obj._id});
                if(existsProd !== null ){
                    let locate = existsProd.products.find(i => i._id == newId);
                    locate.qty++;
                    const res = await cartModel.updateOne({[this.ID_FIELD]:id,"products._id":obj._id},{$set:{"products.$":locate}});
                    return res;
                } else {
                    await cartModel.updateOne({[this.ID_FIELD]:id},{$push:{products:obj}});
                    let existsProd = await cartModel.findOne({[this.ID_FIELD]:id,"products._id":obj._id});
                    let locate = existsProd.products.find(i => i._id == newId);
                    locate.qty = 1;
                    const final = await cartModel.updateOne({[this.ID_FIELD]:id,"products._id":obj._id},{$set:{"products.$":locate}});
                    return final;
                }
            }else{
                return null;
    ...
    async deleteById(id,id_prod){
        try {
            let exists = await cartModel.findById({[this.ID_FIELD]:id,"products._id":id_prod});
            const locate=exists.products.find(x => x._id == id_prod);
            if (locate){
                if(exists.products[0].qty>1){
                    exists.products[0].qty--;
                    const res = await cartModel.updateOne({[this.ID_FIELD]:id,"products._id":mongoose.Types.ObjectId(id_prod)},{$set:{"products.$.qty":exists.products[0].qty}});
                    return res;
                } else{
                    let res = await cartModel.updateOne({[this.ID_FIELD]:id},{$pull : { products : { [this.ID_FIELD] : mongoose.Types.ObjectId(id_prod)}}});
                    return res;
                }
            } else {
                return null;
